...
full_join(Schools13, Schools16, by = c("DistrictID", "SchoolID")) -> TestScores

remove(Schools13, Schools16)
...
TestScores %>% 
  ...
  # mutate(Year = 2000 + as.numeric(substr(Test, nchar(Test) - 1, nchar(Test)))) %>%
  # mutate(Test = substr(Test, 1, nchar(Test)- 8)) %>%
