...
In short, the opposite of what economics is doing with program evaluation, but more inline with engineering approach, i.e., assume randomized assignment and equal probability sampling.

# Why the Effect of Energy Efficiency on Education is Interesting?

+ Purely facilities decision -- money saved on energy.
+ All Energy Efficiency Measures (EEMs) must pass cost benefit test.
+ Improvements in student/staff health and the lifetime effects of academic performance dwarf the value of energy savings.

+ Oregon provides almost perfect framework for Regression Discontinuity Design (RDD).
...
+ Air quality is important but we don't know exactly what part
    + We know air exchanges, $CO_2$ concentrations are important, but beyond that
    + Hard to measure.
    + Allergenic mold concentrations, colony forming bodies per liter, can change by factor of 10 in a few hours.
...
+ [SB 1149 (1999)](https://www.oregonlegislature.gov/bills_laws/lawsstatutes/1999orLaw0865.html) was Oregon's deregulation bill.
    ...
    + Established a Public Purpose Charge that funds thing like The Energy Trust of Oregon (ETO).
...
Makes it easier to establish a natural control group for any treated school by using ineligible ones in the district.
...
+ Eligibility determined by other than educational  outcomes
+ Equality of financial resources reduces inter-district differences.
+ Smaller districts means fewer intra-district differences.

We can feel comfortable using schools out of district for controls given equal funding.
...
Note that some eligible schools already have the Effective EEMs, bias treatment response down.
...
+ Fixtures are described in detail as gym, exterior, LED Exit signs, etc.
...
+ A school can have a mix each year and can have multiple treatments over the years.
+ Installations are frequently but not always during Summer or Winter break.
...
+ Schools in Border Districts with treated schools (Subset for first two Tests): `r nrow(TreatedDistrictSchools)`
...
+ Exams can change from year-to-year.
+ The cut point for passing also changes from year-to-year.
...
+ Regression discontinuity, because of the eligibility requirements, is very close.
+ Room for improving the match of observable differences across treated and untreated schools.

# How Different are Treated, Untreated and Ineligible schools?

Ideally, there should be no _observable_ difference -- the ignorability assumption. As close as you can get to random assignment.
...
The problems start with facility characteristics.

# Problems with Facilities Data

+ Oregon Department of Education (ODE) discontinued the School Facilities Report in 2002. Only have built and remodel dates before 2001.
...
+ Ineligible schools in border districts as control
    + _Regression Discontinuity Design_ (RDD) with student population controls.
    + Refine with a matching method, _Coarsened Exact Matching_ (CEM).
+ Eligible but untreated schools in _all eligible districts_ as controls
    + CEM
...
# Cover Many Different Assumptions about Endogeneity


![](AnalysisPlan.png)

# Simple Regression Discontinuity Design (RDD)
...
+ Intend to Treatment all values of $x > 20$.
...
+ Strong prior that treatment, installing EEMs, is unrelated to student characteristics.
    ...
    + Based solely on energy cost benefit evaluation.
...
+ ELA started in 2015 so test not applicable.
...
Recall, Writing was required for HS only and ELA only has two years of data. 
...
Different Identification technique.

+ Use all eligible schools with eligible but untreated schools as controls.
    + Not just the border districts
    + Not just border districts with treated schools.
+ CEM used as a substitute for IV style identification.
...
ETO guy was right, economists do futz with models.


+ Students were right - there is a there there.
+ I found there is a there there but I don't where it is.
+ RDD alone is insufficient -- changes with CEM are too great to not have a specification error.
+ Joint tests of treatment effects are almost all significant.
+ "Placebos" often have real effects.
    + Collinearity of treatment effects
    + Effective and Placebo often take place at the same time.
+ Attendance is a dud -- probably a heterogeneous effect on students.

# Where Next?

+ Expand the variability in EEM measurement
+ Include Scale of EEMs
    + Expenditures in each broad category, windows, controls, etc. crossed with Effective/Placebo.
    + Not two treatment variables but 12.
    + Allow the measures to degrade according to the measure life.
+ ODE started using a new attendence measure in the final two years.
