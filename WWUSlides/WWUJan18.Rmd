...
#
LogitTransform <- function(x) (x/100)/(1-(x/100))
...
Master %>%
  ...
  mutate(ELALogit  = LogitTransform(ELA.Pct.Met),
         MathLogit = LogitTransform(Math.Pct.Met),
         ReadingLogit  = LogitTransform(Reading.Pct.Met),
         ScienceLogit = LogitTransform(Science.Pct.Met),
         WritingLogit = LogitTransform(Writing.Pct.Met),
...
There are four tests given over the sample period, school years ending 2010 - 2016.
...
+ Year is an important control variable
...
In short, we can't use building characteristics but we can use student characteristics to model treatment process.
...
+ Ineligible schools as control
    + Simple regression with student population controls.
    + Refine with a matching method later.
+ Eligible but untreated schools as controls
    + Must start with matching.

# Is there a there there?

Two Views:

+ Fraction of students passing each of the tests
+ Student Attendance

Note:

+ No contols for student population
+ Only ineligible schools and schools eventually treated.
+ Separate Results for elementary, middle and high schools.
+ Year controls


```{r ThereThere, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
...
OnlyBorder %>% 
  filter(Eligible == TRUE | EverTreated ==TRUE) ->FirstTest

partform <- "EverTreated+Treated + as.factor(Year)"
SimpleResults <- BasicResults(FirstTest, partform)
...
stargazer(SimpleResults[[1]], SimpleResults[[2]], SimpleResults[[3]],SimpleResults[[4]], keep = c("Constant", "Eligible", "Treated"), header = FALSE, type = "latex", dep.var.labels = c("ELA", "Math", "Reading", "Science", "Writing"), column.sep.width = "1pt")
